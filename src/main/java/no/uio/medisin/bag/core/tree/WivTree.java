...
public class WivTree {
  ...
  private ArrayList<TreeNodeGroup> treeGroups = new ArrayList<>();
  ...
  public void getPairDistancesForClassificationNames(String filename) throws IOException{
    ...
    BufferedWriter bwDP =new BufferedWriter(new FileWriter(new File(filename)));
      ...
      ArrayList<String>classificationTypes = this.getSampleClassificationList().getClassificationTypes();
      for(String classificationType: classificationTypes){
        ArrayList<String>classificationNames = this.getSampleClassificationList().getAllEntriessForClassificationType(classificationType);
...
//        logger.debug(currNode.getNodeID() + " | " + currNode.getName());
   ...
   */
  public int addNewNodeGroup(String groupName, int parentNode){
    int lastGroupID = -1;
    int g=0;
    ...
    treeGroups.add(new TreeNodeGroup(lastGroupID + 1, groupName, parentNode));
...
/**
 * get all the nodes underneath this node.
 * 
 * @param cnodeID
 * @return 
 */
  public ArrayList<Integer> getNodesInClade(int cnodeID){
    
    ArrayList<Integer> nodeIDList = new ArrayList<>();
    TreeNode node = this.getNode(cnodeID);
    if(node.isLeafNode()){
      nodeIDList.add(cnodeID);
    }else{
      for(int childID: node.getChildren()){
        nodeIDList.addAll(getNodesInClade( childID));
      }
    }
    return nodeIDList;        
  }  
	  ...
	  if(nextNextrightComma>0 && nextNextrightParenLoc >0) {
		  ...
		  if(nextNextrightComma>0) {
			  ...
			  if(nextNextrightParenLoc >0){
				  ...
				  endOfNumber=queryString.length();
 ...
 */
  ...
  /**
   * print out all nodes in the specified group (no parameter information)
   * @return
   */
  
  public String printNodesInClade(int cladeNodeID){
    String nodeString = "";
	  for(int nodeID: this.getNodesInClade(cladeNodeID)) {
		  nodeString = nodeString.concat(this.getNode(nodeID).getNodeID() + ":" +this.getNode(nodeID).getName() + "+" + StringUtils.repeat("-", 40) + "+\n");
	  }
	  return nodeString;
  }
  ...
  public static void main(String args[]){
      ...
      String badTree = "(EUR___ESN___HG00096:0.11119,(((EUR___FIN___HG00097:0.22229,EUR___GBR___HG00099:0.33339):0.44449,(EUR___FIN___HG00100:0.55559,EUR___GBR___HG00101:0.66669):0.88887):0.77779,EUR___ESN___HG00102:0.88889):0.99991);";
      ...
      try{
        ...
        logger.info("\n\n"+testTree.printNodes());
        Double bl = testTree.findPairDistance("EUR___ESN___HG00096", "EUR___ESN___HG00102");
        bl= 0.00004;
        DecimalFormat df = new DecimalFormat("###.#####");
        System.out.println("BL = " + df.format(bl));
  ...
  /**
   * a simple example to show usage of TreeNodeGroups
   */
  public void treeHouse(){
    // print out all the nodes in the tree
    logger.info(this.printNodes());
    
    /*
      create two groups 
      I'm cheating by looking at the tree, getting the nodeID for each node, 
      and then grabbing the parent ID to define the clade
    */
    //this.addNewNodeGroup("WeaselCatMonkey", this.getNode(this.getNodeID("weasel")).getParentNode());
    //this.addNewNodeGroup("BearRacoon", this.getNode(this.getNodeID("bear")).getParentNode());
      
    for(TreeNodeGroup tng: this.treeGroups){
      int tngID = tng.getGroupID();
      logger.info("\n\n\n\n\n");
      logger.info(tng.getGroupID() + ":" + tng.getGroupName());
      logger.info(this.printNodesInClade(tng.getParentNodeID()));
      logger.info("\n\n\n\n\n");
    }
  }
 }
