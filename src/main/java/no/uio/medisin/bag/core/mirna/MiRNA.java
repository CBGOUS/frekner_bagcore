 ...
 * 
 * 24/4/2021
 * Added a HashMap (isomiRs) to store all the isomiRs associated with a miRNA.
 * The HashMap stores instances of IsomiRString. This is a formatted String that stores
 * key features of an isomiR. It also includes an attributes field to store information 
 * about degenerate isomiRs (i.e., isomiRs that can be generated from more than one
 * miRNA).
 * 
 * Removed the fields to store the predicted secondary structure for the miRNA
 * in the duplex as these can be stored more concisely in bracket notation.
 * 
 ...
 * 
 * To Do:
 * This class needs to be made comparable so the entries stored in a list can be sorted
 * to speed things up
 * 
...
public class MiRNA extends SimpleRNASequence{
    ...
    public HashMap<String, IsomiRString>             isomiRs = new HashMap();
    ...
    */
    ...
    public void parseMiRBaseGFFEntry(String line){
        ...
        try{
            ...
            */
            ...
            super.setChromosome(line.split("\t")[PreMiRNA.COL_CHR].trim());
     ...
     * 
     */
    public void addIsomiR(String name, int start, String cigar, String md, String seq, int count){
        IsomiRString isoMiRStr = new IsomiRString(name, start, cigar, md, seq, count);
        if(isomiRs.containsKey(seq)){
          isoMiRStr.incCount(count);
          isomiRs.computeIfPresent(seq, (k, v) -> isoMiRStr);         
        }else{
          isomiRs.put(seq, isoMiRStr);
        }
        /**
        md = md.replace("\t", "|").trim();
    ...
    public void groupIsomiRsBySeedRegion(int baselinePercent){
      HashMap<String, Integer> seedIsomiRs = new HashMap();
      int totalCounts = this.getTotalIsomiRCounts();
      for(IsomiRString isoStr: isomiRs.values()){
        if((double)isoStr.getCount() / (double) totalCounts > (double)baselinePercent/100.0){
          String isoSeed = isoStr.getSeq().substring(0, 7);
          if(seedIsomiRs.containsKey(isoSeed)){
            int oldCounts = seedIsomiRs.get(isoSeed);
            seedIsomiRs.replace(isoSeed, oldCounts + isoStr.getCount());                            
          }else{
            seedIsomiRs.put(isoSeed, isoStr.getCount());
          }
        }
      }
      this.removeIsomiRs();
      for (Map.Entry<String, Integer> seedIso : seedIsomiRs.entrySet()) {
        // String name, int start, String cigar, String md, String seq, int count
        this.addIsomiR( this.getName(), 0, "", "", seedIso.getKey(), seedIso.getValue());
      }                        
    }
     ...
     */
    public String reportIsomiRsAsTSVs(int baselinePercent, int minCounts){
        ...
        String isomiRStr = "";
        for (IsomiRString iso : this.isomiRs.values()) {
            if((double)iso.getCount() / (double) totalCounts > (double)baselinePercent/100.0){
                isomiRStr = isomiRStr.concat(iso.toTSV());
            }
        }
    ...
    public String prettyReportIsomiRs(int baselinePercent, int minCounts){
        ...
        for(String isomiR: isomiRs){
            ...
            if(Double.parseDouble(isomiR.split(";")[NAMECOL].split("-")[1]) / (double) totalCounts > (double)baselinePercent/100.0){
                ...
                isoString           = isoString.concat(StringUtils.repeat(" ", ColMargin + (isoStart - this.getIsomiRMinStart(baselinePercent))));
                isoString           = isoString.concat(isoSeq);
                isoString           = isoString.concat(StringUtils.repeat(" ", (this.getIsomiRMaxStop(baselinePercent)-(isoStart + isoSeq.length())) + ColMargin));
     ...
     * @return String pretty summary of isomiRs
     */
    public String prettyReportIsomiRs2(int baselinePercent, int minCounts){
        ...
        for (IsomiRString iso : this.isomiRs.values()) {
            if((double)iso.getCount() / (double) totalCounts > (double)baselinePercent/100.0){
                if(iso.getName().length() > longestName)
                    longestName = iso.getName().length();

                if(Integer.toString(iso.getCount()).length() > longestCounts)
                    longestCounts = Integer.toString(iso.getCount()).length();

                if(iso.getSeq().length() > longestSeq)
                    longestSeq = iso.getSeq().length();
                /*
                if(this.getStrand().equals("+")){
                    if(isomiR.split(";")[SEQCOL].equals(this.getSeq().replace("U", "T")))
                        longestName++;
                ...
                */
                ...
                if(iso.getMDString().length() > longestMD)
                    longestMD = iso.getMDString().length();
        ...
        String finalIsoString = "";
        int maxIsoReads = 0;
        for (IsomiRString iso : this.isomiRs.values()) {
            
            if((double)iso.getCount() / (double) totalCounts > (double)baselinePercent/100.0){
                String currIsoString = "";
                currIsoString           = currIsoString.concat(StringUtils.repeat(" ", leftMargin));
                String isoName      = iso.getName();
                if(isoName.contains("iso"))
                  isoName = "*".concat(isoName);
                /*
                if(this.getStrand().equals("+")){
                    if(isomiR.split(";")[SEQCOL].equals(this.getSeq().replace("U", "T")))
                ...
                */
                currIsoString           = currIsoString.concat(StringUtils.repeat(" ", longestName - isoName.length()) + isoName);
                
                int isoStart        = iso.getStart();
                String isoSeq       = iso.getSeq();
                currIsoString           = currIsoString.concat(StringUtils.repeat(" ", ColMargin + (isoStart - this.getIsomiRMinStart(baselinePercent))));
                currIsoString           = currIsoString.concat(isoSeq);
                currIsoString           = currIsoString.concat(StringUtils.repeat(" ", (this.getIsomiRMaxStop(baselinePercent)-(isoStart + isoSeq.length())) + ColMargin));
                
                String isoMD        = iso.getMDString();
                currIsoString           = currIsoString.concat(StringUtils.repeat(" ", ColMargin) + isoMD + StringUtils.repeat(" ", (longestMD - isoMD.length()) + ColMargin));
                String isoCounts    = Integer.toString(iso.getCount());
                int isoCountsINT    = iso.getCount(); 
                DecimalFormat df = new DecimalFormat("##.##%");
                float isoPercentage = (float)isoCountsINT / (float)totalCounts;
                String formattedIsoPercent = df.format(isoPercentage);             
                currIsoString = currIsoString.concat(StringUtils.repeat(" ", ColMargin + (longestCounts - isoCounts.length())) 
                        + isoCounts + StringUtils.repeat(" ", ColMargin)
                        + formattedIsoPercent
                        + StringUtils.repeat(" ", ColMargin) + "\n");
                /*
                now need to insert this in the right place in the string so the counts are ordered
                */
                String isoStrBits[] = finalIsoString.split("\n");

                int matchIndex = 0;
                for(int b=0; b< isoStrBits.length; b++){
                  if(isoStrBits[b].isEmpty())
                    continue;
                  int isoBitCounts = Integer.valueOf(isoStrBits[b].split("\\s+")[4].trim()); 
                  if(isoBitCounts < iso.getCount()){
                    matchIndex = b;
                    break;
                  }
                  matchIndex = b+1; // no mat
                }
                String newFinalString = "";
                for(int bb=0; bb<matchIndex; bb++){
                  newFinalString = newFinalString.concat(isoStrBits[bb] + "\n");
                }
                newFinalString= newFinalString.concat(currIsoString);
                for(int bb=matchIndex; bb<isoStrBits.length; bb++){
                  newFinalString = newFinalString.concat(isoStrBits[bb] + "\n");
                }
               
                finalIsoString = newFinalString;
            
            }
            
        }
        
        if (finalIsoString.equals("")) return "";
        
        return reportStr.concat(finalIsoString + "\n\n\n");
     ...
     */
    public ArrayList characterizeIsomiRsOld(int baselinePercent){
    ...
    public ArrayList characterizeIsomiRs(int baselinePercent){
        ...
        logger.debug("this mirRNA name --- "+super.getName() + "\t" + super.getSeq());
        int m=1;
        for (IsomiRString iso : this.isomiRs.values()) {
        ...
        //for(String isomiR: isomiRStrings){
            ...
            if((double)iso.getCount() / (double) totalCounts > (double)baselinePercent/100.0){
                logger.debug("\n\n[" + m + "]\t" + "" + iso.getName() + "\t" +this.getStrand() +  "\t" + " : percentage - " + (double)iso.getCount() / (double) totalCounts / (double) totalCounts);


                logger.debug(iso.toString());
                int isoStart        = iso.getStart();
                String isoSeq       = iso.getSeq();
                int isoEnd          = isoStart + isoSeq.length() - 1;
                ...
                logger.debug("parent miR - position -- " +this.getSeq() + "\t" + super.getStartPos() + "\t" + super.getEndPos() + "\n");
                logger.debug(SimpleSeq.complement(isoSeq) + "\t" + isoStart + "\t" + isoEnd + "\n");
                logger.debug("miR/ isomiR seq --\n" + " --" + this.getSeq() + "\n" + " --" + isoSeq);
                ...
                if(this.getStrand().equals("+")){
                    ...
                    //logger.debug("noOf 5p/3p Steps + -- "+noOf5pSteps + "/" + noOf3pSteps);
                    //logger.debug("start + end position -- "+isoStart + ", " + isoEnd + ", " + (this.getStartPos()- isoStart) + ", " + this.getSeq().length());
                ...
                isomiRPt.put("3p", noOf3pSteps);
                isomiRPt.put("poly", noOfPolySteps);
                isomiRPt.put("fraction",(double)iso.getCount()/(double)totalCounts);
    ...
    /**
     * sum counts in the isomiR hashmap
     * 
     * @return int total counts
     */
    public int getTotalIsomiRCounts(){
        
        int totalCounts = 0;
        for (IsomiRString iso : this.isomiRs.values()) {
          totalCounts += iso.getCount();
        }      
        return totalCounts;
    }
     ...
     */
    public int getIsomiRMinStart(int baselinePercent){
        ...
        for (IsomiRString iso : this.isomiRs.values()) {
          if((double)iso.getCount() / (double) totalCounts > (double)baselinePercent/100.0){
            if(iso.getStart() < isomiRMinStart) 
                isomiRMinStart = iso.getStart();
          }
        }
     ...
     */
    public int getIsomiRMaxStop(int baselinePercent){
        
        int isomiRMaxStop = -1;
        //String [] isomiRs = isomiRString.split("\t");

         for (IsomiRString iso : this.isomiRs.values()) {
          if((double)iso.getCount() / (double) this.getTotalIsomiRCounts() > (double)baselinePercent/100.0){
            if(iso.getStart() + iso.getSeq().length() > isomiRMaxStop) 
                isomiRMaxStop = iso.getStart() + iso.getSeq().length() - 1;
          }
        }
    ...
    public static void main (String args[]){
        ...
        mirseq.addIsomiR("isomiR1-1000", 3580220, "18M", "XA:i:2\t MD:Z:1C9C6\t NM:i:2", "GAGCGCACCGGACGGGCC", 1);
        mirseq.addIsomiR("isomiR2-100",  3580220, "16M", "XA:i:2\t MD:Z:1C9C6\t NM:i:2", "GTGCGCACCGGACGGGCC", 1);
        mirseq.addIsomiR("isomiR3-60",   3580221, "18M", "XA:i:2\t MD:Z:1C9C6\t NM:i:2",  "AGCGCACCGGACGGGCCT", 1);
        mirseq.addIsomiR("isomiR4-25",   3580221, "19M", "XA:i:2\t MD:Z:1C9C6\t NM:i:2", "AAGCGCACCGGACGGGCC", 1);
        
        logger.error(mirseq.prettyReportIsomiRs(5, 200));

        logger.error(mirseq.getIsomiRMinStart(5));
        logger.error(mirseq.getIsomiRMaxStop(5));
